...
describe("learning modules", () => {
  test("ingests artifacts and builds preflight context", () => {
    ...
    writeFileSync(
      path.join(dir, ".opencode", "plans", "durable-backfill.md"),
      "# Durable Backfill\nPlan body",
    );
    ...
    expect(applied.artifact_refs.length).toBe(2);
    const chunks = db
      .prepare(`SELECT content_type, authority FROM chunk WHERE project_id = ?`)
      .all("p1") as {
      content_type: string;
      authority: number;
    }[];
