...
describe("orchestrator hint", () => {
  ...
  test("appendOrchestratorHint is passive by default", () => {
    const system = ["header"];
    appendOrchestratorHint(system, ORCHESTRATOR_HINT_BLOCK, false);
    expect(system[0]?.includes("<engram-hint>")).toBe(false);
  });

  test("appendOrchestratorHint mutates last chunk when enabled", () => {
    const system = ["header"];
    appendOrchestratorHint(system, ORCHESTRATOR_HINT_BLOCK, true);
