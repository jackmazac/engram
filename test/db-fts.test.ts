...
describe("migrations and FTS", () => {
  test("migrate twice reaches stable user_version", () => {
    ...
    const v1 = Number(
      (d1.query("PRAGMA user_version;").get() as { user_version: number }).user_version,
    );
    d1.close();
    const d2 = openMemoryDb(p);
    const v2 = Number(
      (d2.query("PRAGMA user_version;").get() as { user_version: number }).user_version,
    );
  ...
  test("export checkpoints are scoped by project and root session", () => {
    ...
    const rows = db
      .query(
        `SELECT project_id, exported_count FROM export_checkpoint WHERE root_session_id = ? ORDER BY project_id`,
      )
