...
export class EngramRuntime {
  ...
  constructor(input: RuntimeInput, cfg: EngramConfig) {
    ...
    this.pFrictionLatest = d.prepare(
      `SELECT report FROM friction_cache ORDER BY time_created DESC LIMIT 1`,
    );
    ...
    this.pUpdateRetrievalRefs = d.prepare(
  ...
  onMessageUpdated(ev: { properties?: { info?: Record<string, unknown> } }) {
    ...
    const correlation = correlationFromUnknown(ev.properties, info);
    const rows = capture.fromAssistantError(
      info,
      this.input.project.id,
      this.cfg,
      this.planSlug.get(sid) ?? null,
      correlation,
    );
    this.enqueue(rows);
  }
  ...
  onPartUpdated(ev: { properties?: { part?: Record<string, unknown> } }) {
    ...
    const correlation = correlationFromUnknown(ev.properties, part);
    const rows = capture.fromPart(
      part,
      this.input.project.id,
      this.cfg,
      this.planSlug.get(sid) ?? null,
  ...
  onToolAfter(tool: string, sessionID: string, output: string, metadata?: unknown) {
    const correlation = correlationFromUnknown(metadata);
  ...
  private insertRows(batch: ChunkInsert[]) {
    ...
    const tx = this.db.transaction((rows: ChunkInsert[]) => {
      for (const r of rows) {
        ...
        insertChunkCorrelation(this.db, { chunk_id: r.id, correlation: r.correlation });
        seen.add(key);
        inserted++;
      }
    });
  ...
  private existingChunkKeys(batch: ChunkInsert[]): Set<string> {
    ...
    const clauses = Array.from(wanted.values())
      .map(
        () =>
          `(session_id = ? AND message_id = ? AND coalesce(part_id, '') = ? AND content_hash = ?)`,
      )
      .join(" OR ");
    const args: string[] = [this.input.project.id];
    for (const r of wanted.values())
      args.push(r.session_id, r.message_id, r.part_id ?? "", r.content_hash);
    ...
    return new Set(
      rows.map(
        (r) =>
          `${this.input.project.id}\0${r.session_id}\0${r.message_id}\0${r.part_key}\0${r.content_hash}`,
      ),
    );
  }
  ...
  async memoryTool(
    ...
    try {
      ...
      if (
        status === "ok" &&
        this.cfg.telemetry.logSlowOperations &&
        durationMs >= this.cfg.telemetry.slowMs
      ) {
  ...
  async injectSystem(sessionID: string | undefined, system: string[]): Promise<void> {
    ...
    if (this.cfg.context.proactiveHints.enabled !== true) return;
  ...
  private async applyOrchestratorHint(sessionID: string, system: string[]) {
    ...
    appendOrchestratorHint(
      system,
      ORCHESTRATOR_HINT_BLOCK,
      this.cfg.context.proactiveHints.enabled === true,
    );
  }
  ...
  runLifecycleIngest(opts: {
    ...
    const worktree = opts.worktree_root ? path.resolve(opts.worktree_root) : this.input.worktree;
    ...
    return {
      applied: summary.dryRun === false,
      discovered: summary.discovered,
      ingested: summary.chunksInserted,
      skipped: Math.max(0, summary.items - summary.chunksInserted),
      artifact_refs: summary.artifact_refs,
      lifecycle_object_ids: summary.lifecycle_object_ids,
...
Archives: ${arch.c} | export checkpoints: ${ck.c} | backfill_done: ${bf?.v ?? "0"}${memSz}${hotSz}`;
    ...
    if (report === "telemetry") {
      ...
      return [
        ...
        formatEventReport(
          recentLogEvents(this.db, pid, { limit: 50, minLevel: "info" }),
          "last 50",
        ),
      ].join("\n\n");
    }
  ...
  private memoryChunkCount(): number {
    const row = this.db
      .prepare(`SELECT count(*) AS c FROM chunk WHERE project_id = ?`)
      .get(this.input.project.id);
    return isCountRow(row) ? row.c : 0;
  }
...
function workspaceSignalsFromCorrelation(correlation: EngramCorrelation | null | undefined) {
  ...
  return {
    workspaceId: stringOrUndefined(correlation.workspace_id),
    planId: stringOrUndefined(correlation.plan_id),
    planSlug: stringOrUndefined(correlation.plan_slug),
    waveId: stringOrUndefined(correlation.wave_id),
    agentRunId: stringOrUndefined(correlation.agent_run_id),
    correlationId: stringOrUndefined(correlation.correlation_id),
    toolCallId: stringOrUndefined(correlation.tool_call_id),
    spineSeq: correlation.spine_seq ?? undefined,
    artifactRef: stringOrUndefined(correlation.artifact_ref),
    lifecycleObjectId: stringOrUndefined(correlation.lifecycle_object_id),
    concordEventId: stringOrUndefined(correlation.concord_event_id),
  };
}

function workspaceSignalsFromArgs(
  args: ConflictContextArgs,
  correlation: EngramCorrelation | null,
) {
  const base = workspaceSignalsFromCorrelation(correlation);
  return {
    ...base,
    workspaceId: args.workspace_id ?? base?.workspaceId,
    planId: args.plan_id ?? base?.planId,
    planSlug: args.plan_slug ?? base?.planSlug,
    waveId: args.wave_id ?? base?.waveId,
    agentRunId: args.agent_run_id ?? base?.agentRunId,
    correlationId: args.correlation_id ?? base?.correlationId,
...
function contextMode(raw: string | undefined): ContextMode | undefined {
  if (
    raw === "plan" ||
    raw === "implement" ||
    raw === "review" ||
