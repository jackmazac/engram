...
import { chunkCorrelationFiltersFromContext, queryChunksByCorrelation } from "./db.ts";
...
export async function searchMemory(opts: {
  ...
  const correlatedIds = queryChunksByCorrelation(
    opts.db,
    chunkCorrelationFiltersFromContext(opts.correlation),
  );
  const merged = applyFeedbackBoosts(
    opts.db,
    opts.projectId,
    rrfMerge([correlatedIds, ftsRanked, vecRanked]),
  );
...
function applyFeedbackBoosts(
  ...
  return [...ranked]
    .map((r) => {
      ...
      const typeBoost =
        a &&
        ["decision", "api_contract", "invariant", "plan", "analysis", "bug"].includes(
          a.content_type,
        )
          ? 0.01
          : 0;
...
function buildMetrics(
  input: Omit<SearchMetrics, "totalMs"> & { totalStart: number },
): SearchMetrics {
  return {
...
async function rerank(
  ...
  const placeholders = ids.map(() => "?").join(",");
  const chunks = db
    .prepare(`SELECT id, content FROM chunk WHERE id IN (${placeholders})`)
    .all(...ids) as {
    id: string;
    content: string;
  }[];
  ...
  const pack = ids
    .map((id, i) => `${i + 1}. [${id}] ${(byId.get(id)?.content ?? "").slice(0, 400)}`)
    .join("\n");
