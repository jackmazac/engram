...
/** Fleet passivity default: hints are opt-in so Engram returns evidence without steering agents. */
export function appendOrchestratorHint(system: string[], block: string, enabled: boolean) {
  if (!enabled) return;
