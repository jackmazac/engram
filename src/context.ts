...
function distillationCandidates(
  db: Database,
  projectId: string,
  terms: string[],
  limit: number,
...
function perSectionLimit(section: ContextSectionId, mode: ContextMode, limit: number): number {
  ...
  if (section === "current_risks" && (mode === "review" || mode === "debug" || mode === "audit"))
    return 5;
...
function routeSection(mode: ContextMode, c: Candidate): ContextSectionId {
  if (["decision", "api_contract", "invariant", "product_requirement", "pattern"].includes(c.type))
    return "must_know";
  if (mode === "plan" && ["bug", "analysis", "test_strategy", "perf_note"].includes(c.type))
    return "current_risks";
  if (mode === "review" && ["bug", "test_strategy", "error", "analysis"].includes(c.type))
    return "current_risks";
  if (mode === "debug" && ["bug", "error", "perf_note"].includes(c.type)) return "current_risks";
  if (mode === "audit" && ["analysis", "bug", "product_requirement"].includes(c.type))
    return "current_risks";
  if (["migration", "test_strategy", "perf_note"].includes(c.type)) return "prior_successful_paths";
  if (
    c.source === "root" ||
    c.source === "distillation" ||
    c.type === "plan" ||
    c.type === "milestone"
  )
...
function modeTypeBoost(
  mode: ContextMode,
  type: string,
  source: ContextBundleItem["source"],
): number {
  const plan = new Set([
    "decision",
    "api_contract",
...
function expandTerms(query: string, signals: WorkspaceSignals | undefined): string[] {
  ...
  for (const file of signals?.changedFiles ?? [])
    for (const term of tokenize(file)) terms.add(term);
  if (signals?.branch) for (const term of tokenize(signals.branch)) terms.add(term);
  for (const term of correlationTerms(signals)) terms.add(term);
  return [...terms].filter((x) => x.length > 1).slice(0, 32);
}

function correlationTerms(signals: WorkspaceSignals | undefined): string[] {
  if (!signals) return [];
  return [
